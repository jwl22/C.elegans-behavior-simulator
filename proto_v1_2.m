...
textLabel_ci = uilabel(fig, 'Text', 'Initial C. elegans Adults Number:', 'Position', [textlabel_x, text_label_y, textlabel_width, textlabel_height], 'FontSize', 13, 'FontWeight', 'bold');
...
for i=24*3*3+1:steps
    ...
    if move_num > 0
        ...
        xyz_pos = mvnrnd([x_cumul(i,move_idx)+migration_speed/3.*cos(phi).*cos(theta), y_cumul(i,move_idx)+migration_speed/3.*cos(phi).*sin(theta), z_cumul(i,move_idx)+migration_speed/3.*sin(phi)], migration_speed.*ones(1,move_num*3));
